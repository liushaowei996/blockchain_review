...
A \emph{link/path} is a time-varying end-to-end route composed of its physical media, intermediate nodes, security associations, and quality state. A data product may traverse a UUV acoustic hop, a USV gateway, a satellite backhaul, and a cloud broker; confidentiality, freshness, availability, and routing integrity depend on this composed path. A \emph{data product} is an immutable version of raw observation, feature set, track, map, fused estimate, model output, command, alert, provenance record, or audit artifact. Treating versions as immutable avoids silent overwriting and permits provenance to express transformation.
